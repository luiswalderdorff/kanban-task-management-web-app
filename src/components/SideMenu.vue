<template>
  <div v-if="$store.state.sideMenuOpen" class="side-menu-container">
    <template v-if="!modalOpen || !$store.state.mobile">
      <div class="side-menu">
    ...
    <modal-component :content="modalOpen">
      <h2 class="heading-large">Add New Board</h2>
      <board-form @close-event="modalOpen = false" />
    </modal-component>
  </div>
</template>
...
export default {
  ...
  watch: {
    windowWidth: {
      handler(val) {
        const tabletView = val >= 768;
        this.$store.state.sideMenuOpen = tabletView;
        this.$store.state.mobile = !tabletView;
      },
      immediate: true,
    },
  },
...
.board-button {
  ...
  &:not(.board-button--active):hover {
    background-color: var(--main-purple-hover-light);
